...
const BuscarNormasBO = props => {
    ...
    const [form, setForm] = useState({ ...initForm, idNormasEstadoTipo: 1 });
    ...
    const handleFormChange = (e) => {
        ...
        switch (e.target.name) {
            ...
            case 'idReparticionOrganismo':
                ...
                setForm({
                    ...form,
                    ['idReparticionOrganismo']: parseInt(value)
                })
                break;
            case 'userCarga':
                value = e.target.value;
    ...
    const getCuentas = async () => {
        ...
        try {
            ...
            let token = localStorage.getItem("token");
            await ApiPinPost('/api/v1/boletin-oficial/cuentas', body, token).then((res) => {
                setCuentas(Array.from(res.data.cuentas))
            }).catch(function (error) {
                setLoading(false)
                throw error
            });
    ...
    const getOrganismos = async () => {
        ...
        try {
            let token = localStorage.getItem("token");
            await ApiPinPost('/api/v1/boletin-oficial/organismos-emisores', { idUsuario: localStorage.getItem('idUsuarioBO') }, token)
                .then((res) => {
            ...
            <table class="table table-bordered table-hover" >
                <thead>
                    <tr>
                        ...
                        <th scope="col">Boletin Número</th>
                        ...
                        <th scope="col"></th>
                        <th scope="col"></th>
                        <th scope="col"><input type="checkbox" aria-label="checkbox" checked={checkedNormas.length === normas.length} onClick={(e) => { checkAll(e, normas) }} /></th>
                    </tr>
                </thead>
                <tbody>
                    {
                                    ...
                                    <td>
                                        <div class="dropdown-container">
                                            <div class="drowdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    <button class="dropdown-item btn-sm" onClick={() => navigate(n.idNorma + "/" + rutasBO.cotizar, { state: { idNorma: n.idNorma } })}>
                                                        Cotizar
                                                    </button>
                                                    <button class="dropdown-item btn-sm" data-toggle="modal" data-target="#modalFechaLimite" type="button"
                                                        onClick={() => { setNormaCambiarFechaLimite(n) }}>
                                                        Cambiar Fecha Límite
                                                    </button>
                                                    {n.idNormaAviso !== null &&
                                                        <button class="dropdown-item btn-sm" onClick={() => window.open("normas/" + n.idNorma, '_blank')}>
                                                            Ver aviso asociado
                                                        </button>
                                                    }
                                                    <button class="dropdown-item btn-sm" data-toggle="modal" data-target="#modalAsociarAviso" type="button"
                                                        onClick={() => { setNormaAsociarAviso(n) }}>
                                                        Asociar con Aviso
                                                    </button>
                                                </div>
                                            </div>
        ...
        return (
            <>
                <div className="container responsive mb-5" id="pagina-buscar-normas">
                                ...
                                <div className="card-body">
                                    <div className="filtros-busqueda">
                                        <form className="form" onSubmit={e => handleSubmit(e)} id="form-buscar-normas">
                                            ...
                                            <div className="form-group" id="contenedor">
                                                ...
                                                ><option selected value=""></option>
                                                    {cuentas && (cuentas.length > 0) ? (
                                                        cuentas.map((n, index) => (
                                                            <option value={n.idCuenta} key={'opt-sec-' + index}>{n.nombre}</option>
                                                        ))
                                                    ) : null}
                                                </select>
                                            </div>
                                            ...
                                            <div className="form-group" id="contenedor">
                                                ...
                                                ><option selected value=""></option>
                                                    {reparticion && (reparticion.length > 0) ? (
                                                        reparticion.map((n) => (
                                                            <option value={n.idReparticion} key={n.idReparticion}>{n.reparticion}</option>
                                                        ))
                                                    ) : (<option>No hay reparticiones</option>)}
                                                </select>
                                            </div>
                                            ...
                                            <ExportarExcel ruta={'/api/v1/boletin-oficial/normas/exportar'} form={form} />
                                        </form>
                                    </div>
                                </div>
                            </div>
                        ...
                        normas && (normas.length > 0) ? (
                            <div class="resultados">
                                <div class="titulo">
                                    ...
                                    {checkedNormas.length > 0 &&
                                        <button class="btn btn-success mb-1" data-toggle="modal" data-target="#modalFechaLimite">
                    ...
                    {/* El mismo modal sirve para una sola o varias normas al mismo tiempo */}
                    <ModalFechaLimite checkedNormas={checkedNormas} normaCambiarFechaLimite={normaCambiarFechaLimite} setNormaCambiarFechaLimite={setNormaCambiarFechaLimite} />
                    {normaAsociarAviso && <ModalAsociarAviso norma={normaAsociarAviso} setNorma={setNormaAsociarAviso} />}
                </div>
            </>
        );
