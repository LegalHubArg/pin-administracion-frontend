...
const Abrogacion = props => {
  ...
  const [patologiasNormativas, setPatologiasNormativas] = useState([])
  ...
  const handleFormChange = (e) => {
    ...
    switch (e.target.name) {
      ...
      case 'idAbrogacionTipoPasiva':
        ...
        break;
      case 'solucionAdoptada':
        value = e.target.value;
        setForm({ ...form, ['solucionAdoptada']: value })
        break;
      case 'observaciones':
        value = e.target.value;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await ApiPinPost('/api/v1/dj/analisis-epistemologico/formulario1/guardar', body, token)
        .then(_ => {
      ...
      setLoading(false)
      setModalError({ show: true, mensaje: e.data.e.mensaje ? e.data.e.mensaje : 'Ocurrió un error al guardar el formulario.' })
  ...
  const getFormulario1 = async () => {
    ...
    let token = localStorage.getItem("token");
    try {
      const formulario = await ApiPinPost('/api/v1/dj/analisis-epistemologico/formulario1', body, token)
        .catch((err) => {
          throw err
        });
      if (formulario.data.data.length === 1) {
        ...
        setForm({
          ...
          solucionAdoptada: data.solucionAdoptada,
  ...
  useEffect(async () => {
    ...
    const patologias = await getPatologiasNormativas().catch(err => { })
    setPatologiasNormativas([...patologias])
    const tiposAbrogacion = await getTiposAbrogacion().catch(err => { })
    setTiposAbrogacion([...tiposAbrogacion])
    await getNorma()
    await getFormulario1()
    setLoading(false)
  }, [])
      ...
      <>
        <div className="container responsive mb-5" id="abrogacion-expresa">
          ...
          <form className="form" onSubmit={e => handleSubmit(e)}>
            ...
            <h3>Abrogada/Derogada Expresamente por:</h3>
            <div className="card">
              <div className="card-body formulario-asiento">
                <div className="form-group buscar">
                  <label for="asiento">id Norma</label>
            ...
            <div className="form-group" style={{ width: "fit-content" }}>
              <label for="idPatologiaNormativa">Patología Normativa</label>
              <select className="custom-select" id="idPatologiaNormativa" name="idPatologiaNormativa"
                onChange={e => handleFormChange(e)} value={form.idPatologiaNormativa} disabled
              >{patologiasNormativas && patologiasNormativas.length > 0 ?
                <option selected value={form.idPatologiaNormativa}>{patologiasNormativas.find(elem => elem.idPatologiaNormativa === form.idPatologiaNormativa).nombre}</option>
                : <option selected hidden>No hay patologías normativas</option>}
              </select>
            </div>
            <div className="form-group">
              <label for="observaciones">Solución adoptada</label>
              <textarea className="form-control" id="solucionAdoptada" name="solucionAdoptada" rows="4"
                onChange={e => handleFormChange(e)} value={form.solucionAdoptada}
              />
            </div>
            <div className="form-group">
