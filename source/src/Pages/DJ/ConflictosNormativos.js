...
const ConflictosNormativos = props => {
  ...
  const handleFormChange = (e) => {
    ...
    switch (e.target.name) {
      case 'idPatologiaNormativa':
        value = parseInt(e.target.value);
        if (isNaN(value)) {
          value = null
        }
        setForm({ ...form, ['idPatologiaNormativa']: value })
        break;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await ApiPinPost('/api/v1/dj/analisis-epistemologico/formulario2/guardar', body, token)
        .then(_ => {
  ...
  const getFormulario2 = async () => {
    ...
    let token = localStorage.getItem("token");
    try {
      const formulario = await ApiPinPost('/api/v1/dj/analisis-epistemologico/formulario2', body, token)
        .catch((err) => {
          throw err
        });
      if (formulario.data.data.length === 1) {
        ...
        setForm({
          idPatologiaNormativa: data.idPatologiaNormativa,
  ...
  if (isLoading) {
    ...
    return (
      <>
        <div className="container responsive mb-5" id="conflictos-normativos">
          <h3>Norma:</h3>
          <div className="card">
          ...
          <form className="form" onSubmit={e => handleSubmit(e)}>
            ...
            <h3>En conflicto normativo con:</h3>
            <div className="card">
              <div className="card-body formulario-asiento">
            ...
            <div className="form-group" style={{ width: "fit-content" }}>
              ...
              <select className="custom-select" id="idPatologiaNormativa" name="idPatologiaNormativa"
                onChange={e => handleFormChange(e)} value={form.idPatologiaNormativa ? form.idPatologiaNormativa : -1}
              >{patologiasNormativas && patologiasNormativas.length > 0 ?
                patologiasNormativas.map((elem) =>
                  <option value={elem.idPatologiaNormativa}>{elem.nombre}</option>)
