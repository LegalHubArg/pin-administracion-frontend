...
const NecesidadRefundir = props => {
  ...
  const handleFormChange = (e) => {
    ...
    switch (e.target.name) {
      ...
      case 'textoUnificado':
        value = e.target.value;
        setForm({ ...form, ['textoUnificado']: value })
        break;
      case 'idCausal':
        value = parseInt(e.target.value);
        if (isNaN(value)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await ApiPinPost('/api/v1/dj/analisis-epistemologico/formulario4/guardar', body, token)
        .then(_ => {
  ...
  const getFormulario4 = async () => {
    ...
    let token = localStorage.getItem("token");
    try {
      const formulario = await ApiPinPost('/api/v1/dj/analisis-epistemologico/formulario4', body, token)
        .catch((err) => {
          throw err
        });
      if (formulario.data.data.length === 1) {
        ...
        setForm({
          ...
          textoUnificado: data.textoUnificado,
  ...
  if (isLoading) {
    ...
    return (
      <>
        <div className="container responsive mb-5" id="necesidad-refundir">
          <h3>Norma:</h3>
          <div className="card">
          ...
          <form className="form" onSubmit={e => handleSubmit(e)}>
            ...
            <div className="form-group" style={{ width: "fit-content" }}>
              ...
              <select className="custom-select" id="idCausal" name="idCausal" disabled
                onChange={e => handleFormChange(e)} value={form.idCausal !== null ? form.idCausal : 2}
              >
                {causales && causales.length > 0 ?
                  causales.map((n) => (n.idCausal === 2) ? (
                    <option selected value={n.idCausal}>{n.nombre}</option>
                  ): null ) : <option selected hidden>No hay causales</option>}
              </select>
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              <label for="observaciones">Texto unificado</label>
              <textarea className="form-control" id="textoUnificado" name="textoUnificado" rows="4"
                onChange={e => handleFormChange(e)} value={form.textoUnificado}
              />
            </div>
            <br />
            <h3>Documento</h3>
...
export default NecesidadRefundir;